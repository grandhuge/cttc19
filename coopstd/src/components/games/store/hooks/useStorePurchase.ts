...
export const useStorePurchase = (
  ...
  const simulateCustomerPurchase = () => {
    ...
    if (Math.random() < purchaseChance) {
      ...
      // แก้ไขบัค - ไม่เพิ่มเงินสะสมจากการขายในร้าน
      updateGameState({
  ...
  // เปลี่ยนให้เกมร้านค้าทำงานต่อเนื่องแม้สลับหน้า
  useEffect(() => {
    // Define isInStoreGame outside the interval to avoid scope issues
    ...
    const interval = setInterval(() => {
      ...
      // เพิ่มโอกาสขายเมื่ออยู่ในเกมร้านค้า
      if (isInStoreGame) {
        baseSalesChance *= 1.5; // เพิ่มโอกาส 50% เมื่ออยู่ในเกม
      }
    ...
    }, isInStoreGame ? 2500 : 5000); // ช้าลงเมื่อไม่อยู่ในเกม
