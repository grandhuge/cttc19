// ตัวแปรสำหรับเกม
        ...
        saveScoreButton.addEventListener('click', async () => {
            ...
            try {
                const result = await saveScoreToAPI(
                    playerName,
                    score.toFixed(1),
                    correctCount,
                    wrongCount,
                ...
                if (result.status === 'success') {
                    ...
                    saveStatusMessage.textContent = 'เกิดข้อผิดพลาด: ' + (result.message || 'ไม่ทราบสาเหตุ');
                    saveStatusMessage.className = 'mt-2 text-sm text-red-500';
                    showNotification('บันทึกคะแนนไม่สำเร็จ: ' + (result.message || 'ไม่ทราบสาเหตุ'), 'error');
                }
            } catch (error) {
                saveStatusMessage.textContent = 'เกิดข้อผิดพลาดในการเชื่อมต่อ: ' + error.message;
                saveStatusMessage.className = 'mt-2 text-sm text-red-500';
